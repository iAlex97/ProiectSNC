...
\subsubsection {Comentarii referitor grafice obtinute }
...
\subsection {Caracteristici semnal SPAB de intrare }
\begin{table}[H]
  \centering
  \begin{tabular}{|l|l|l|l|}
    \hline
...
\subsection {Afisare spectrul semnal SPAB de intrare }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{2_3_1.eps}}
\end{center}

\subsection {Observatii asupra semnalului SPAB generat }
...
\subsection {Afisare spectrul semnal SPAB de iesire (achizitionat) }

\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{2_7_1.eps}}
\end{center}

\subsection {Observatii asupra semnalului achizitonat }
...
\subsection {Rezolvare Aplicatii Lab 4 }
...
\end{lstlisting}
\subsubsection {Spectru semnale filtrate: }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{3_3_2.eps}}
\end{center}
\subsubsection {Comentarii asupra spectrului }
...
\end{lstlisting}
\subsection {Estimarea complexității model ARX }
\begin{lstlisting}
...
\end{center}
\subsection {Identificare model ARMAX }
\begin{lstlisting}
...
\end{center}
\subsection {Identificare model BJ }
\begin{lstlisting}
...
\end{center}
\subsection {Identificare model OE }
...
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{3_7_3_2.eps}}
\end{center}
\subsubsection {Studiul stabilității sistemului: }
...
\subsection {Pregătire date inițiale WINPIM. Fisierul txt obtinut (upload) }
Fisierul txt poate fi gasit la urmatorul path: $./modele\_winpim/B70\_date\_win.txt$

\subsection {Incarcare fisier in WinPIM si specificare perioada de esantionare }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{winpim/winpim_incarcare_1.png}}
\end{center}

\subsection {Aplicare filtrare set de date (eliminarea componentei continue) }
...
\subsection {Estimarea complexitatii}
...
\subsection {Identificare si validare modele }
...
\begin{center}
  \begin{gather*}
  x^{T} =[ 1\ s_{1} \ \vdots \ r_{0} \ r_{1}]\\
  p^{T} =[ 1\ p_{1} \ p_{2} \ \vdots \ 0 ]
  \end{gather*}
\end{center}
...
Asadar, rezulta polinoamele R si S:
\begin{center}
  \begin{equation*}
  \begin{cases}
  R\left( q^{-1}\right) =-1.2517+0.8955q^{-1}\\
  S\left( q^{-1}\right) =1-0.2251q^{-1}
  \end{cases}
  \end{equation*}
\end{center}

\subsection {Pentru ca sistemul sa ofere timp de raspuns minim si suprareglaj max. 5\% se aleg:}

Se alege generatorul de traiectorii $B_m/A_m$ astfel incat sa indeplineasca conditiile din cerinta:

\begin{center}
  \begin{equation*}
  \frac{B_{m}\left( q^{-1}\right)}{A_{m}\left( q^{-1}\right)} =\frac{1.0000+0.75941^{-1}}{1.0000-1.3193q^{-1} +0.4382q^{-2}}
  \end{equation*}
\end{center}
...
\begin{center}
  \begin{equation*}
  T\left( q^{-1}\right) =0.9604-1.2671q^{-1} +0.4209q^{-2}
  \end{equation*}
\end{center}
...
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{matlab/5_10_1.png}}
\end{center}

Simularea sistemului:
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{matlab/5_10_2.png}}
\end{center}

\subsection {Observatii legate de rezultatele obtinute: }
\begin{itemize}
  \item Urmareste referinta, dar are eroare stationara nenula
  \item Indeplineste performantele supuse regimului tranzitoriu
  \item Nu rejecteaza perturbatiile de tip treapta aparute
\end{itemize}
...
Rezulta, similar, polinoamele R si S:
\begin{center}
  \begin{equation*}
  \begin{cases}
  R\left( q^{-1}\right) =9.7548-11.1478q^{-1}+3.0818q^{-2}\\
  S\left( q^{-1}\right) =1-0.2251q^{-1}-0.7749q^{-2}
  \end{cases}
  \end{equation*}
\end{center}
...
\subsection {Simulare sistem in bucla inchisa (comanda, referinta, iesirea), in conditii de perturbatii treapta (25\% amplitudine) aplicate dupa stabilizarea sistemului fata de referinta. Graficele sunt prezentate aici: }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{matlab/5_16_1.png}}
\end{center}

\subsection {Observatii legate de rezultatele obtinute: }
\begin{itemize}
  \item Urmareste referinta si are eroare stationara nula
  \item Indeplineste performantele supuse regimului tranzitoriu
  \item Rejecteaza perturbatiile de tip treapta aparute
\end{itemize}
...
\begin{table}[H]
  \centering
    \begin{tabular}{|l|l|}
      \hline
       $R(q^{-1})$ & $0.007993-0.897038q^{-1}+0.993165q^{-2}$\\
      \hline
       $S(q^{-1})$ & $1-0.75029q^{-1}-0.24971q^{-2}$\\
      \hline
       $T(q^{-1})$ & $14.204545-26.200142q^{-1}+12.099716q^{-2}$\\
      \hline
       $B_m(q^{-1})/A_m(q^{-1})$ & $(0.03048 + 0.02556q^{-1})/(-1.533754q^{-1}+0.589793q^{-2})$ \\
      \hline
    \end{tabular}
...
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{winreg/6_4_1.png}}
\end{center}

Simulare cu perturbatia anterioara si perturbatii stocastice:
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{winreg/6_4_2.png}}
\end{center}
...
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{7_1_2_1.eps}}
\end{center}

\subsubsection {Alegand o alta referinta raspunsul sistemului este capturat in figura de mai jos: }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{7_1_2_2.eps}}
\end{center}
...
\subsection {Comentarii privind calitatea solutiei obtinute vs specificatiile impuse: }
\begin{itemize}
...
\subsection {Regulatorul RST 1 si-a indeplinit sau nu performantele impuse ? Daca nu, ce masuri se iau (ce specificatii noi se impun fata de primul design ) ? }
Regulatorul RST 1 nu si-a indeplinit performantele impuse asupra timpului tranzitoriu si a suprareglajului cand a fost testat pe instalatie. Eroarea stationara este aproximativa cu zero, iar perturbatiile stochastice sunt rejectate.

\subsection {Regulatorul RST 1 indeplineste marginile standard de robustete (se pot verifica cu aplicatia WinPIM)? Figura cu functia de sensibilitate si template este furnizata aici. }
\begin{center}
  \fbox{ \includegraphics[width=0.8\textwidth]{8_3_1.png}}
\end{center}

Se observa ca sistemul are marginea de amplitudine si de faza destul de bune pentru a asigura robustetea sistemului in bucla inchisa.

\subsection {In cazul in care regulatorul a trebuit recalculat acesta este descris de polinoamele: }
\subsection {Rezultatele in simulare sunt furnizate in figura urmatoare: }
\subsection {Functia de sensibilitate a noii solutii: }

\section {Evaluare Experimentala Regulator RST-2 }
a
\subsection {Evaluare performante pe sistemul real. }
...
\begin{itemize}
  \item Sistemul aproximat nu se comporta intocmai ca cel real, obtinand rezultate diferite fata de simularile din WinReg/Matlab
\end{itemize}

\subsection {Feedback legat de desfasurare/ continut proiect }
\begin{itemize}
  \item Aplicatia web OneNote este greu de folosit pentru utilizatorii care nu au acces la un client nativ, in special pe linux/mac.
\end{itemize}
